...
import { fetchBoardData, updateBoardName } from "@/redux/feautres/boardSlice.js";
...
const saveBoardName = (boardId, boardName, dispatch) => {
  socket.emit("updateBoardInWorkspace", { boardId, updateData: { boardName } }, (response) => {
    if (!response) {
...
export default function GroupsDisplay({ module, workspaceId, boardId }) {
  ...
  if (loading) {
    return <div className="text-gray-500 ml-32 text-lg italic pt-7 flex items-center justify-center">Loading Groups...</div>;
  }
    ...
    <div className="bg-gray-50 px-16 py-8 rounded-xl shadow-lg">
      ...
      <div className="space-y-8">
        {boardData.groups.map((group) => (
          <div key={group.groupId} className="p-6 bg-white border border-gray-200 rounded-lg shadow-md hover:shadow-lg cursor-pointer">
            <Group module={module} boardType={boardType} boardId={boardId} group={group} isAdmin={isAdmin} />
          </div>
        ))}
      </div>
    </div>
