...
export default function DeleteGroupButton({ groupId, isAdmin }) {
  ...
  return (
    <div className="relative group">
      <button
        onClick={isAdmin ? handleDeleteGroup : () => {}}
        disabled={!isAdmin}
        className={`p-2 rounded-md shadow-md transition duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-1 ${buttonClasses}`}
      >
