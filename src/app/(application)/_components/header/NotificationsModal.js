...
export default function NotificationsModal({ isOpen, setIsOpen, setUnreadCount, notifications, setNotifications, bgColor }) {
  ...
  return (
    <div className={`fixed right-0 top-16 w-[30rem] rounded-l-3xl bg-zinc-800 text-white shadow-lg h-full z-50 p-5 overflow-y-auto transition-transform duration-1000 ease-in-out transform ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
      <div className="mb-20">
        ...
        {notifications.length === 0 && <p className="mt-10 text-gray-400 text-center">No notifications.</p>}
