...
export default function DevServiceDashboard({ module, userId, workspaceId }) {
  ...
  useEffect(() => {
    ...
    socket.emit("getDashboardDetails", { moduleId: Cookies.get("moduleId"), userId, workspaceId }, (response) => {
      ...
      const parsedData = relevantStats.map((key) => processChartData(response[key], key)).filter(Boolean);
  ...
  return (
    <div className="min-h-20 p-6 pb-10 bg-gray-100">
      <h1 className="text-3xl font-bold mb-6 text-gray-800 text-center">{moduleName} Dashboard</h1>

      <div className="flex justify-center mb-10 space-x-4">
        {chartsData.map(({ key }) => (
          <button key={key} className={`px-6 py-3 rounded-lg border transition-all duration-200 ${activeTab === key ? "bg-blue-600 text-white border-blue-600 shadow-md" : "bg-white text-gray-800 border-gray-300 hover:bg-gray-100 hover:shadow-sm"}`} onClick={() => setActiveTab(key)}>
