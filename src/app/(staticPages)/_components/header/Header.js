...
export default function Header({ userDetails }) {
  ...
  return (
    <header className="bg-white text-gray-800 h-16 flex items-center justify-between px-6 md:px-24 sticky top-0 z-50 shadow-md w-full">
      <h1 className="text-slate-800 text-2xl md:text-3xl font-extrabold hover:text-purple-400 transition-colors duration-300 cursor-pointer">
        WorkFlow
      </h1>
          ...
          <nav className="flex flex-col items-center py-4 space-y-4">
            {navLinks.map((link) => (
              <Link
                ...
                className={`text-lg ${pathname === link.href ? "text-purple-600 font-bold" : "hover:text-purple-400"}`}
              >
                {link.label}
              </Link>
            ))}
